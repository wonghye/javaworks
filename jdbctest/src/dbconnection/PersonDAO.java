...
//DAO data access object - db작업을 수행하는 코드
public class PersonDAO {
	...
	//CURD - create, read, update, delete   + 로그인
	//자료 삽입
	...
	public ArrayList<Person> getPersonList(){
		...
		try {
			...
			rs = pstmt.executeQuery();  // 데이터 반환 받음
			while(rs.next()) { // 자료가 있는 동안 반복
				Person person = new Person();
				person.setUserId(rs.getString("userid"));  //db테이블의 칼럼 가져옴
	...
	public Person getPerson(String userId) {
		...
		try {
			...
			if(rs.next()) {  // 자료가 있으면
				person.setUserId(rs.getString("userid")); //db에 있는 user id 가져옴
				person.setUserPw(rs.getNString("userpw"));
				person.setName(rs.getString("name"));
				person.setAge(rs.getInt("age"));
			}
		} catch (SQLException e) {
